...
async def test_index_points_scope_order_replacement_and_explicit_retry(rag):
    ...
    records, _ = await rag.store.client.scroll(rag.store.collection, limit=100)
    ...
    assert await rag.store.search(8, 18, 1, rag.embedding.embed_query("q"), 5) == []
    blog.get_version_chunks.return_value = source(0)
    await rag.index_article_version(18, 1, access_token="token", blog_client=blog)
    assert await rag.store.search(7, 18, 1, rag.embedding.embed_query("q"), 5) == []
    assert (await rag.store.client.count(rag.store.collection)).count == 4
...
async def test_embedding_failure_preserves_existing_points(rag, monkeypatch, failure):
    ...
    await rag.index_article_version(18, 1, access_token="token", blog_client=blog)
    previous, _ = await rag.store.client.scroll(rag.store.collection, with_vectors=True)
    ...
    monkeypatch.setattr(rag.embedding, "embed_documents", embed)
    delete = AsyncMock(wraps=rag.store.client.delete)
    monkeypatch.setattr(rag.store.client, "delete", delete)
    with pytest.raises(RagError):
        await rag.index_article_version(18, 1, access_token="token", blog_client=blog)
    delete.assert_not_called()
    after, _ = await rag.store.client.scroll(rag.store.collection, with_vectors=True)
...
async def test_upsert_failure_can_be_repaired_by_explicit_reindex(rag, monkeypatch):
    ...
    upsert = rag.store.client.upsert
    broken = AsyncMock(side_effect=RuntimeError("fake-secret"))
    monkeypatch.setattr(rag.store.client, "upsert", broken)
    ...
    monkeypatch.setattr(rag.store.client, "upsert", upsert)
    await rag.index_article_version(18, 1, access_token="token", blog_client=blog)
    assert (await rag.store.client.count(rag.store.collection)).count == 3
...
async def test_go_authorizes_before_embedding_or_qdrant(rag, monkeypatch, error_type):
    ...
    monkeypatch.setattr(rag.embedding, "embed_documents", docs)
    monkeypatch.setattr(rag.embedding, "embed_query", query)
    monkeypatch.setattr(rag.store, "replace", replace)
    monkeypatch.setattr(rag.store, "search", search)
...
async def test_filters_top_k_domain_mapping_and_stale_content_rejection(rag, monkeypatch):
    ...
    monkeypatch.setattr(rag.store, "ensure_collection", AsyncMock(return_value=True))
    query = AsyncMock(return_value=QueryResponse(points=points))
    monkeypatch.setattr(rag.store.client, "query_points", query)
    rag.top_k = 3
    ...
    assert kwargs["limit"] == 3 and kwargs["with_vectors"] is False
...
async def test_incompatible_collection_rejected_before_delete_or_query(rag, monkeypatch, mismatch):
    profile = dict(rag.store.profile)
    if mismatch in ("model", "provider"):
        profile[mismatch] = "different"
    await rag.store.client.create_collection(
        rag.store.collection,
    ...
    with pytest.raises(RagConfigurationError):
        await rag.store.replace(source(1), rag.embedding.embed_documents(["text"]))
    with pytest.raises(RagConfigurationError):
        await rag.store.search(7, 18, 1, rag.embedding.embed_query("q"), 5)
    delete.assert_not_called()
...
async def test_read_does_not_create_collection_and_index_creates_payload_indexes(rag, monkeypatch):
    assert await rag.store.search(7, 18, 1, rag.embedding.embed_query("q"), 5) == []
    assert not await rag.store.client.collection_exists(rag.store.collection)
    create_index = AsyncMock()
    monkeypatch.setattr(rag.store.client, "create_payload_index", create_index)
    await rag.store.ensure_collection(create=True)
...
def test_provider_profile_must_match_store():
    ...
    with pytest.raises(RagConfigurationError):
        ArticleRagService(embedding, store, top_k=5)
