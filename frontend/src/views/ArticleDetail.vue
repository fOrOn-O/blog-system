...
function canDeleteComment(comment) {
  return isAdmin.value || authStore.currentUser?.id === (comment?.user_id || comment?.user?.id)
}
      ...
      <article class="article">
        ...
        <header class="article-header">
          ...
          <div v-if="canDeleteArticle" class="article-actions">
            <button v-if="isAuthor" class="action-btn" @click="router.push(`/article/edit/${articleId}`)">
            ...
            <button v-if="canDeleteArticle" class="action-btn danger" @click="handleDeleteArticle">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
